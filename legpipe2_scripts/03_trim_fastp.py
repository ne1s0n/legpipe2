...
CORES=7 #number of parallel threads
DRY_RUN=False #if True commands are just printed but not executed
MAX_SAMPLES=float("inf") #maximum number of samples to process, for test purposes
SKIP_PREVIOUSLY_COMPLETED=True #if TRUE samples already processed will be skipped
...
#and the outfolder
def create_filenames(infile_R1, outfolder):
...
with ThreadPool(CORES) as pool:
	for result in pool.starmap(trim, args.itertuples(index = False)):
